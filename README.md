...
[![Version](https://img.shields.io/badge/version-v0.9.73-blue)](https://github.com/ARC-Game-Club/EndstoneMC-ARC-Core-Plugin)
...
- **附件 NBT 摘要** - 富物品附件（潜影盒、附魔物品等）在邮件详情的附件行下方显示**内容物前 4 项**（自定义名优先）与**附魔前 4 条中文名**，超出部分显示「等X项」（需 `arc_inventory ≥ 0.2.1`，缺失时静默跳过；收纳袋内容为组件化存储暂不支持）
